...
fun main() {
    ...
    val cardOpts: Map<Char, Int> = mapOf('A' to 14, 'K' to 13, 'Q' to 12, 'J' to 1, 'T' to 10, '9' to 9, '8' to 8, '7' to 7, '6' to 6, '5' to 5, '4' to 4, '3' to 3, '2' to 2)
 ...
 */
fun categorizeCard(card: String, wildcard: Char = 'J'): Int {
    ...
    val numWildcard: Int = cardSorted.getOrDefault(wildcard, 0)
    ...
    when(topCard) {
        ...
        // 4 of a kind val 5
        4 -> {
            // Wildcard increase to 5 of a kind
            if (numWildcard > 0) return 6
            return 5
        }
        3 -> {
            // If 2 wildcards full house becomes 5 of a kind
            if (numWildcard == 2) return 6

            // If 3 wildcards and 2 of another card five of a kind
            val secondCard: Int = cardSorted.values.toIntArray()[1]
            if (numWildcard == 3 && secondCard == 2) return 6

            // Wildcard increase to 4 of a kind
            if (numWildcard > 0) return 5
        ...
        2 -> {
            // If 2 wildcards and 2 other cards 4 of a kind
            val secondCard: Int = cardSorted.values.toIntArray()[1]
            if (numWildcard == 2 && secondCard == 2) return 5

            // Full House
            if (numWildcard == 1 && secondCard == 2) return 4

            // 3 of a kind
            if (numWildcard > 0) return 3
